...
function sineWave(canvas, imageData) {
  ...
  const amplitude = Math.floor(canvas.height / verticalRepeats);
  // per line
  ...
  for (let i = 0; i < imageData.data.length; i += 4) {
    //const gray = i / imageData.data.length * 255;
  ...
  for (let x = 0; x < canvas.width * lines; ++x) {
    ...
    for (let y = 0; y < amplitude; ++y) {
      ...
      blend(imageData, ii, z);
      //const z = Math.random() * 255;//Math.round(zVal) === y ? 255 : 0;
...
function CanvasChart({
  ...
  useEffect(() => {
    ...
    // Define the sine function parameters
